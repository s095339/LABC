...
void parser_jpg_top(ap_uint<AXI_WIDTH>* datatoDDR,
    ...
    b1 = segment[0](7, 0);
    b2 = segment[0](15, 8);
    if ((size < 127) | (b1 != 0xFF) | (b2 != 0xD8)) { //find SOI(start of image) header.
    ...
    while (!scanned && !rtn) {
        ...
        } else if (b2 == 0xDB) { // marker (Define Quantization Table)DQT
...
#pragma HLS PIPELINE
        ...
        } else if (b2 == 0xC0) { // marker (start of frame)
...
#pragma HLS PIPELINE
        ...
        } else if (b2 == 0xC4) { // marker  //DHT(Define Huffman table)
...
// for print
        ...
        } else if (b2 == 0xDA) { // marker  //SOS (start of scan)
